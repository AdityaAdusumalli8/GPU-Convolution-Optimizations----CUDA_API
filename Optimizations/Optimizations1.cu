...
__global__ void conv_forward_kernel(float *output, const float *input, const float *mask, const int B, const int M, const int C, const int H, const int W, const int K,const int S)
{
   ...
   */
   ...
   if (h < H_out && w < W_out && by < M) {
       float value = 0.0f;
       __half conv_result = __float2half(0.0f);
       for (int c = 0; c < C; c++) {
           for (int p = 0; p < K; p++) {
               for (int q = 0; q < K; q++){
                    if(((h*S+p<H) && (w*S+q<W))){
                        conv_result= __hadd(conv_result, __hmul(__float2half(in_4d(bx,c,h*S+p, w*S+q)), __float2half(mask_4d(by,c,p,q))));
                    }

                    // WeightMatrix  Optimization
                    /*int maskIndex = by * (C * K * K) + c * (K * K) + p * K + q;
                    value += in_4d(bx, c, h*S+p, w*S+q) * mask[maskIndex];
                    */ 
               }
                    
            }
        }
       out_4d(bx, by, h, w) = __half2float(conv_result);
    }
